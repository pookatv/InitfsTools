...
// in DLLmain.cpp inside the injected DLL and calls back over the pipe.
//
...
{
public:
    PipeReaderThread(ConsoleWindow* wnd, ConsoleWindow::PipeBridge* bridge)
        : m_wnd(wnd), m_bridge(bridge), m_collectingList(false)
    {
    }
...
private:
    void run() override
    {
        while (m_bridge->hPipe != INVALID_HANDLE_VALUE)
        {
            ...
            if (packet.empty())
            {
                QMetaObject::invokeMethod(m_wnd, "appendOutputLine",
                    Qt::QueuedConnection,
                    Q_ARG(QString, QStringLiteral("[Bridge] Pipe disconnected.")),
                    Q_ARG(bool, false),
                    Q_ARG(bool, true));
                QMetaObject::invokeMethod(m_wnd, [wnd = m_wnd]() {
                    ...
                    wnd->m_lblCmdCount->setText(QStringLiteral("0 commands"));
            ...
            // A batch packet contains multiple \n-separated sub-packets
            size_t start = 0;
    ...
    void processPacket(const std::string& packet)
    {
        ...
        {
            m_collectingList = true;
            // Do NOT clear m_listLines/m_listDescs — append vars to the existing methods
            return;
        }

        if (m_collectingList && packet.size() >= 7 && packet.compare(0, 7, "RESULT:") == 0)
        {
            // "list" command complete — publish the collected names and descriptions.
            m_collectingList = false;
            QStringList finalList = m_listLines;
            QStringList finalDescs = m_listDescs;
            // Pad descs to match names length in case some entries lacked descriptions
        ...
        if (m_collectingList)
        {
            // Handle METHOD: and VAR: lines directly
            if (packet.size() >= 7 && packet.compare(0, 7, "METHOD:") == 0)
            {
                std::string text = packet.substr(7);
                // Format: "Group.Name\tDescription" or just "Group.Name"
                ...
                while (!desc.empty() && (desc.back() == '\r' || desc.back() == '\n'))
                    desc.pop_back();
                const char* np = name.c_str(); int nn = (int)name.size();
                const char* dp = desc.c_str(); int dn = (int)desc.size();
                QString entryName = QString::fromUtf8(np, nn).trimmed();
                QString entryDesc = QString::fromUtf8(dp, dn).trimmed();
            ...
            {
                std::string text = packet.substr(4);
                size_t tab = text.find('\t');
                std::string name = (tab != std::string::npos) ? text.substr(0, tab) : text;
                std::string desc = (tab != std::string::npos) ? text.substr(tab + 1) : std::string();
                while (!name.empty() && (name.back() == '\r' || name.back() == '\n' || name.back() == ' '))
        ...
        if (packet.size() >= 7 && packet.compare(0, 7, "RESULT:") == 0)
        {
            ...
            const char* p = text.c_str();
            int         n = (int)text.size();
            line = QString::fromUtf8(p, n);
            if (text.find("Unknown console command") != std::string::npos)
                isError = true;
            else
                isResult = true;
        }
        else if (packet.size() >= 7 && packet.compare(0, 7, "OUTPUT:") == 0)
        {
            size_t pipe = packet.find('|', 7);
            ...
            else if (tag.empty() && !text.empty() && text[0] != '[')
            {
                // Empty tag + doesn't start with '[' = direct game output result, show green
                isResult = true;
            }
        ...
        else if (packet.size() >= 13 && packet.compare(0, 13, "UNLOCK_AVAIL:") == 0)
        {
        ...
        else if (packet.size() >= 14 && packet.compare(0, 14, "UNLOCK_RESULT:") == 0)
        {
            ...
            QMetaObject::invokeMethod(m_wnd, [wnd = m_wnd, result]()
                {
                    if (result != 1)
                    {
                        // Patch failed — revert button state
        ...
        else if (packet.size() >= 6 && packet.compare(0, 6, "ERROR:") == 0)
        {
...
void ConsoleWindow::buildUi()
{
    ...
    // ── Toolbar ───────────────────────────────────────────────────────────────
    {
        ...
        m_btnClear = makeBtn(QStringLiteral("Clear"), 60);
        m_btnCopy = makeBtn(QStringLiteral("Copy Log"), 76);
    ...
    auto* noteLabel = new QLabel(
        QStringLiteral("Note: inject dxgi.dll must be placed next to the game exe, or use the normal inject path."),
        m_overlay);
    ...
    connect(m_btnCopy, &QPushButton::clicked, this, &ConsoleWindow::onCopyLog);
...
void ConsoleWindow::onSendCommand()
{
    ...
    // Echo to log
    {
        ...
        QTextCharFormat fmt;
        fmt.setForeground(m_colEchoFg);
        cur.setCharFormat(fmt);
        cur.insertText(QStringLiteral("> ") + text + QStringLiteral("\n"));
...
// ─────────────────────────────────────────────────────────────────────────────
void ConsoleWindow::appendOutputLine(const QString& line, bool isResult, bool isError)
{
    QTextCursor cur = m_txtLog->textCursor();
    cur.movePosition(QTextCursor::End);
    ...
    // Split embedded newlines so QPlainTextEdit's block count stays accurate.
    const QStringList parts = line.split(QLatin1Char('\n'));
    for (int i = 0; i < parts.size(); ++i)
    {
        if (i > 0) cur.insertText(QStringLiteral("\n"));
...
void ConsoleWindow::onMethodListReceived(const QStringList& methods, const QStringList& descs)
{
    ...
    for (int i = 0; i < methods.size(); ++i)
    {
        ...
        if (!m_commandNames.contains(name, Qt::CaseInsensitive))
        {
    ...
    m_lblCmdCount->setText(
        QString::number(m_commandNames.size()) + QStringLiteral(" commands"));
}
...
void ConsoleWindow::onCopyLog()
{
    QApplication::clipboard()->setText(m_txtLog->toPlainText());
    m_lblStatus->setText(QStringLiteral("Copied"));
}
...
void ConsoleWindow::onInjectToProcess()
{
    ...
    if (pid == 0)
    {
        ...
        appendOutputLine(
            ...
            QStringLiteral("\n[Inject] Launch the game first, then click Inject again."),
            false, true);
        m_targetExePath.clear();
        return;
    }
    ...
    {
        appendOutputLine(
            QStringLiteral("[Inject] dxgi.dll not found next to initfstools.exe"),
            false, true);
        m_btnInject->setEnabled(true);
        m_targetExePath.clear();
        return;
                ...
                QThread* connectThread = QThread::create([bridge, self, exeNameCopy]()
                    {
                            ...
                            QMetaObject::invokeMethod(self, [self]()
                                {
                                    self->appendOutputLine(
                                        QStringLiteral("[Inject] Pipe connect failed — proxy DLL may not have a host waiting yet. Try again."),
                                        false, true);
                        ...
                        for (;;)
                        {
                            ...
                            QString line = QString::fromUtf8(packet.c_str(), (int)packet.size());
                            QMetaObject::invokeMethod(self, [self, line]()
                                {
                                    self->appendOutputLine(line, false, false);
                                    if (line.contains(QStringLiteral("[Inject] Injecting into:")))
                                        self->hideOverlay();
                                }, Qt::QueuedConnection);
                        }
                        ...
                        QMetaObject::invokeMethod(self, [self, readyQ, exeNameOnly]()
                            {
                                ...
                                {
                                    self->appendOutputLine(
                                        QStringLiteral("[Inject] Pipe closed before READY"),
                                        false, true);
                                    self->m_bridge.disconnect();
                                    self->m_btnInject->setEnabled(true);
                                    return;
                                ...
                                {
                                    bool ok = false;
                                    int cmdCount = readyQ.mid(6).trimmed().toInt(&ok);
                                    if (!ok) cmdCount = 0;
                                    ...
                                    self->appendOutputLine(
                                        ...
                                        false, false);
                                    self->appendOutputLine(
                                        QStringLiteral("[Inject] ") + QString::number(cmdCount) +
                                    ...
                                    self->m_bridge.startReading(self);
                                }
                                else
                                {
                                    self->appendOutputLine(
    ...
    QThread* connectThread = QThread::create([bridge, self, exeNameCopy]()
        {
            ...
            for (;;)
            {
                ...
                // Pre-READY diagnostic packet — display immediately on the main thread.
                QString line = QString::fromUtf8(packet.c_str(), (int)packet.size());
                QMetaObject::invokeMethod(self, [self, line]()
                    {
                        self->appendOutputLine(line, false, false);
            ...
            QMetaObject::invokeMethod(self, [self, readyQ, exeNameOnly]()
                {
                    ...
                    // Only re-enable (and keep visible) on failure paths.
                    // On success, m_btnInject is hidden above; the toolbar
                    // button should not be visible while connected.
                    if (!self->m_txtInput->isEnabled())
                        self->m_btnInject->setEnabled(true);
                }, Qt::QueuedConnection);
        });
...
void ConsoleWindow::onToggleUnlock()
{
    ...
    // We therefore: confirm → write a flag file → restart the process → auto-inject.
...
// ── Step 2: open with full rights as soon as the exe module is mapped
            ...
            for (;;)
            {
                ...
                QString line = QString::fromUtf8(packet.c_str(), (int)packet.size());
                QMetaObject::invokeMethod(self, [self, line]()
                    { self->appendOutputLine(line, false, false); }, Qt::QueuedConnection);
            }

            QString readyQ = QString::fromUtf8(ready.c_str(), (int)ready.size());
            QString exeNameCopy = exeNameOnly;
            QMetaObject::invokeMethod(self, [self, readyQ, exeNameCopy, savedExePath]()
                {
                    if (!readyQ.startsWith(QStringLiteral("READY:")))
                    {
                        self->appendOutputLine(
                    ...
                    bool ok2 = false;
                    int cmdCount = readyQ.mid(6).trimmed().toInt(&ok2);
                    if (!ok2) cmdCount = 0;

                    self->appendOutputLine(
                        QStringLiteral("[Unlock] Connected after restart — ") + exeNameCopy, false, false);
                    self->appendOutputLine(
                        QStringLiteral("[Unlock] ") + QString::number(cmdCount) +
                        QStringLiteral(" console commands registered."), false, false);
                    self->appendOutputLine(
                        QStringLiteral("[Unlock] All commands unlocked (patches applied at startup)."),
                        false, false);
                    ...
                    // Request full command + var lists from the now-unlocked game
                    std::string listCmd("__LIST__");
                    self->m_bridge.sendCommand(listCmd);
                    std::string varsCmd("__LIST_VARS__");
                    self->m_bridge.sendCommand(varsCmd);
...
std::string ConsoleWindow::PipeBridge::readPacket()
{
    ...
    if (!ok && GetLastError() == ERROR_IO_PENDING)
    {
        ...
        if (wait != WAIT_OBJECT_0) { CloseHandle(ov.hEvent); return {}; }
...
void ConsoleWindow::PipeBridge::startReading(ConsoleWindow* wnd)
{
    // If a previous thread is somehow still alive, clean it up first.
    if (readerThread)
    {
        readerThread->quit();
    ...
    readerThread = new PipeReaderThread(wnd, this);
    readerThread->start();
}
...
void ConsoleWindow::applyTheme(bool dark)
{
    ...
    setStyleSheet(QStringLiteral(
        ...
        // Scrollbars — match ReferenceLibWindow exactly
    ...
    // Panel borders — white in dark mode, black in light (matches ReferenceLibWindow)
...
// ─────────────────────────────────────────────────────────────────────────────
// applyDarkWindowTitle — Win32 DWM dark caption bar
// ─────────────────────────────────────────────────────────────────────────────
