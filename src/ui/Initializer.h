...
public:
    ...
    // Called from main() before show(), if a file was passed on the
    // command line (double-click / "Open with" / drag onto the exe icon)
    void setPendingFileToLoad(const QString& path) { m_pendingFileToLoad = path; }
signals:
